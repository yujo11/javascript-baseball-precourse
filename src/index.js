// 컴퓨터가 뽑는 숫자 생성
function comNumber() {
  const distinctNumbers = new Set(); // 중복을 제거하기 위한 Set
  while (distinctNumbers.size < 3) {
    const com = Math.floor(Math.random() * 9) + 1; // 1 - 9 까지의 숫자 랜덤 생성
    distinctNumbers.add(com);
  }
  return Array.from(distinctNumbers); // set으로 뽑힌 값 배열로 반환
}
...
function answer(e) {
  if (e.type === "click" || (e.type === "keyup" && e.key === "Enter")) {
    const inputValue = input.value; // 입력값 변수에 저장
    input.value = ""; //입력후 초기화
    if (
      inputValue.length !== 3 || // 입력값 길이 체크
      new Set(inputValue).size !== 3 || // 입력값 중복 체크
      isNaN(inputValue) // 입력값이 숫자로 변환시 NaN일시 true 반환
    ) {
      alert("유효하지 않는 입력값");
      input.value = ""; // 잘못된 입력값이면 입력값 비우기
      return;
    }

    const inputArray = Array.from(inputValue).map(Number); // 배열화 후 문자열인 요소들 숫자로 재배치
    const strikeCount = threeNumber.filter(
      (el, i) => el === inputArray[i]
    ).length;
    let findBall = 0;
    for (let i = 0; i < 3; i++) {
...
input.addEventListener("keyup", answer); //키보드
button.addEventListener("click", answer); //마우스 클릭
