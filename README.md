...
- 함수 실행 흐름
  1. 컴퓨터 난수 생성
  2. 사용자가 submit 시, 게임 시작
  - 사용자 입력값 받아오기
  - 입력값과 컴퓨터 값을 비교하여 결과 메시지 출력
  - 정답을 맞췄다면, 게임을 다시 시작할 것인지 물어보기
- 함수 목록

  - index.js

    - 컴퓨터 3자리 난수 생성 함수
    - 게임을 재시작할 것인지 물어보는 html 요소 생성 함수
    - 게임 재시작 버튼을 누르면, 게임을 초기화하는 함수

  - check_user_numbers.js

    - { export }사용자 입력값에 오류가 있다면 경고창을 띄우고, 없다면 해당 값을 배열로 받아오는 함수
      - 입력값이 3자리이고, 숫자인지 확인하는 함수
      - 입력값에 중복이 없는지 확인하는 함수

  - count.js
    - strike 갯수 카운트하는 함수, ball 갯수 카운트하는 함수, 낫싱인지 판별하는 함수
    - strike 메시지 만드는 함수, ball 메시지 만드는 함수, 낫싱 메시지 만드는 함수
    - { export } 위의 3개를 합쳐 최종 메시지 만드는 함수
